...
#region EditorChecks
		...
		private void OnDestroy()
		{
			...
			inited = false;
            if(AFPSCounter.streamWriter != null)
            {
                AFPSCounter.streamWriter.Close();
                AFPSCounter.streamWriter.Dispose();
                AFPSCounter.streamWriter = null;
            }
		}
#endregion
		...
		private IEnumerator UpdateFPSCounter()
		{
			while (true)
			{
                ...
                if (AFPSCounter.streamWriter != null)
                {
                    String strLine = DateTime.Now.GetDateTimeFormats('s')[0].ToString();
                    strLine += "\tFPS:" + fps;
                    strLine += "\tTotalmemory:" + memoryCounter.lastTotalValue;
                    strLine += "\tAllocmemory:" + memoryCounter.lastAllocatedValue;
                    strLine += "\tmono:" + memoryCounter.lastMonoValue;
                    strLine += "\tmono_Used:"  + UnityEngine.Profiling.Profiler.GetMonoUsedSizeLong();
                    strLine += "\tmono_Total:" + UnityEngine.Profiling.Profiler.GetMonoHeapSizeLong();
                    AFPSCounter.streamWriter.WriteLine(strLine);
                    AFPSCounter.streamWriter.Flush();
                }
			}
		}
